/*
 ...
 ================================================================
  ...
  Change history:
  25/02/16  M. A. Cervera  V1.0  
     Initial version - Based on igrf2011_matlab_wrapper.c (V1.1), 
     updated to use IGRF2016 model from IRI2016

  06/03/19  M. A. Cervera  V1.1 
     Additional check of inputs to make sure NaN, Inf or complex
...
void igrf2016_calc_(float *glat, float *glong, float *dec_year, float *height, 
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (nrhs != 4) {
    mexErrMsgTxt("ERROR: 4 inputs required when calling igrf2016.");
    return;
  }

  if (nlhs != 1) {
    mexErrMsgTxt("ERROR: 1 outputs required when calling igrf2016.");
    return;
  }
  ...
  igrf2016_calc_(&glat, &glong, &dec_year, &height, &dipole_moment, &babs, 
