/*
 ======================================================================
                           iri2020_matlab_wrapper.c
 ======================================================================
  Matlab wrapper for iri2020_calc_ of iri2020
       
  This is a MEX-file for MATLAB.
      
  Change history:
  20/04/2020  M. A. Cervera V1.0 
      Initial version - based on iri2016_matlab_wrapper.c (V1.2)
...
#define max_num_hts 1000   /* limitation of IRI2020 */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (nrhs != 4 && nrhs != 7 && nrhs != 8 ) {
    mexErrMsgTxt("ERROR: 4, 7, or 8 inputs required when calling iri2020.");
    return;
  }

  if (nlhs != 2) {
    mexErrMsgTxt("ERROR: 2 outputs required when calling iri2020.");
    return;
  }
  ...
  /* read the date extents of the ig_rz.dat file */
#if !defined(WIN64)
  ...
  /* Copy the RHS pointers (input from MATLAB) to the input variables of
     ...
     iri2020 is to consider it to be UT otherwise iri2020 considers dhour to be
     local time. */
  ...
  if (nrhs >= 5) {
    ...
    /* restrict num_hts to valid range of values for IRI2020*/
  ...
  jf[42] = 1;
  jf[43] = 1;
  jf[44] = 0;   /* currently unused */
  jf[45] = 0;   /* currently unused */
  jf[46] = 0;
  jf[47] = 1;
  jf[48] = 1;   
  jf[49] = 0;   /* currently unused */
  ...
  if (R12 > 0 && R12 <= 200) {
    ...
    /* Calculate IG12, F10.7 and F10.7_81 from the user input R12 and put into
     ...
     irisub.for fortran code from IRI2020 and was verified against 
  ...
  if (nrhs == 8) {
	...
	*(oarr + 0) = foF2;
	...
	else if (strcmp(tmp_str, "FPT-2018") == 0) jf[23] = 0;
	else invalid_field_value_flag = 1; 
      }
      else invalid_field_value_flag = 1;
    }
    ...
    /* read in CGM computation field */
    tmp_ptr = mxGetField(prhs[nrhs-1], (mwIndex) 0, "CGM_comp");
    if (tmp_ptr != NULL) {
      num_valid_fields = num_valid_fields + 1;
      if (mxIsChar(tmp_ptr)) {
	mxGetString(tmp_ptr, tmp_str, (mwSize) 50);
	if (strcmp(tmp_str, "on") == 0) jf[46] = 1; 
	else if (strcmp(tmp_str, "off") == 0) jf[46] = 0;
	else invalid_field_value_flag = 1; 
      }
      else invalid_field_value_flag = 1;
    }

    /* read in Ti field - Ion temperature model*/
    tmp_ptr = mxGetField(prhs[nrhs-1], (mwIndex) 0, "Ti_model");
    if (tmp_ptr != NULL) {
      num_valid_fields = num_valid_fields + 1;
      if (mxIsChar(tmp_ptr)) {
	mxGetString(tmp_ptr, tmp_str, (mwSize) 50);
	if (strcmp(tmp_str, "Tru-2021") == 0) jf[47] = 1; 
	else if (strcmp(tmp_str, "Bil-1981") == 0) jf[47] = 0;
	else invalid_field_value_flag = 1; 
      }
      else invalid_field_value_flag = 1;
    }
    
    /* read in plasmasphere field - plasmasphere model up to the plasmapause*/
    tmp_ptr = mxGetField(prhs[nrhs-1], (mwIndex) 0, "plasmasphere");
    if (tmp_ptr != NULL) {
      num_valid_fields = num_valid_fields + 1;
      if (mxIsChar(tmp_ptr)) {
	mxGetString(tmp_ptr, tmp_str, (mwSize) 50);
	if (strcmp(tmp_str, "Ozhogin") == 0) jf[48] = 1; 
	else if (strcmp(tmp_str, "Gallagher") == 0) jf[48] = 0;
	else invalid_field_value_flag = 1; 
      }
      else invalid_field_value_flag = 1;
    }
  ...
  if (num_valid_fields != num_fields) {
    mexPrintf("Warning: IRI2020 - Some of the fields of the supplied iri_options structure\n");
  ...
  iri2020_calc_(jf, &jmag, &glat, &glon, &year, &mmdd, &dhour, &heibeg, &heiend,
                &heistp, outf, oarr);

  /* Create matricies for the return arguments. We only want to retain the 
     first 15 x num_hts outputs of outf */
  outf_M_keep = 15;
  outf_N_keep = num_hts;
