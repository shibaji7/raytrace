/* 
  ==============================================================================
                   raytrace-2d-sp_matlab_wrapper.c
  ==============================================================================
   Matlab wrapper for raytrace_2d_sp_
        
   This is a MEX-file for MATLAB.
       
   Change history:
   21/04/10  M.A.Cervera  V1.0  Author
   ...
   26/05/16  M.A. Cervera V1.6  Updated for multi-threaded raytrace_2d_sp
...
void raytrace_2d_sp_(int *num_rays, double *elev, double *bearing,
		     double *freqs, int *nhops, double *step_size_min,
		     double *step_size_max, double *tol, double *radius_earth,
...
{
  int mrows, ncols, arrsize, in_arrsize[15], ii, jj, kk, idx, initialize_iono;
  int mrows7, mrows8, mrows9, ncols7, ncols8, ncols9, ncols13, num_bytes;
  ...
  FILE *fopen(), *file_ptr;
  const mwSize *dims;
  const int ray_data_numfields = 21;
  const char *ray_data_fieldnames[] =
     {"ground_range", "group_range", "phase_path",
  ...
  /* Input parameters to raytrace_2d_sp_ */
  double  *elevs;               /* initial ray elevation (deg) */
  double  bearing;              /* ray bearing (deg) */
  double  *freqs;               /* carrier frequency or rays (MHz) */
  ...
  double  radius_earth;         /* radius of the Earth km */
  ...
  /* Output parameters from raytrace_2d_sp_ */
  ...
  in_arrsize[0] = 1;    /* array of initial elevations for rays (>= 1) */
  in_arrsize[1] = 1;    /* ray bearing */
  in_arrsize[2] = 1;    /* ray carrier frequency for rays (>= 1) */
  in_arrsize[3] = 1;    /* number of hops to perform */
  in_arrsize[4] = 1;    /* 1 or 3, ODE solver tol and min/max stepsize */
  in_arrsize[5] = 1;    /* radius of Earth */
  in_arrsize[6] = 1;    /* irregs flag */
  in_arrsize[7] = max_num_ht*max_num_rng;   /* iono en grid */
  in_arrsize[8] = max_num_ht*max_num_rng;   /* iono en grid 5 min later */
  in_arrsize[9] = max_num_ht*max_num_rng;   /* collision freq. grid */
  in_arrsize[10] = 1;                       /* start height of iono grids */
  in_arrsize[11] = 1;                       /* height increment of iono grids */
  in_arrsize[12] = 1;                       /* range increment of iono grids */
  in_arrsize[13] = 4*max_num_rng;           /* irregularities strength grid */
  in_arrsize[14] = 9;                       /* input ray state vector */

  /* Check for proper number of arguments. */
  if (nrhs != 7 & nrhs != 8 & nrhs != 14 & nrhs != 15) {
    mexErrMsgTxt("ERROR: incorrect number of arguments");
    return;
  }

  /* determine if ionosphere needs to be initialized */
  ...
  if (nrhs == 7 || nrhs == 8) {
    ...
    /* OK since we have (nrhs == 8 || nrhs == 9) then an ionosphere has not
       ...
       raytrace_2d_sp call has already passed it in. If not return an error */
    if (!iono_exist_in_mem) {
      mexErrMsgTxt("ERROR: ionosphere does not exist in memory from a prior raytrace_2d_sp call.");
      return;
    } else {
      initialize_iono = 0;  /* use ionosphere from prior call to raytrace_2d_sp */
    }
  ...
  /* make sure inputs 0 to 6 have the correct size */
  for (ii = 0; ii < 7; ii++) {
    mrows = mxGetM(prhs[ii]);
    ncols = mxGetN(prhs[ii]);
    arrsize = mrows*ncols;
    if (ii == 0 & arrsize != 0) arrsize = 1;  /* must be >= 1 */
    if (ii == 2 & arrsize != 0) arrsize = 1;  /* must be >= 1 */
    if (ii == 4 & arrsize == 3) arrsize = 1;  /* may be 1 or 3 elements */
  ...
  if (initialize_iono) {
    ...
    /* make sure inputs 7, 8 and 9 are not too large */
    for (ii = 7; ii < 10; ii++) { 
      mrows = mxGetM(prhs[ii]);
      ncols = mxGetN(prhs[ii]);
    ...
    /* make sure inputs 10, 11 and 12 have the correct size */
    for (ii = 10; ii < 13; ii++) {
      mrows = mxGetM(prhs[ii]);
      ncols = mxGetN(prhs[ii]);
      arrsize = mrows*ncols;
    ...
    /* make sure array size of inputs 7, 8, 9 and 13 are consistent */
    mrows7 = mxGetN(prhs[7]);
    mrows8 = mxGetN(prhs[8]);
    mrows9 = mxGetN(prhs[9]);
    if (mrows8 - mrows7 != 0 || mrows9 - mrows7 != 0) {
      snprintf(error_mesg, 200, "ERROR: Number of rows of inputs 8, 9 and 10 are inconsistent.");
      mexErrMsgTxt(error_mesg);
      return;
    }
    ...
    ncols13 = mxGetN(prhs[13]);
    if (ncols8 - ncols7 != 0 || ncols9 - ncols7 != 0 || ncols13 - ncols7 != 0) {
      snprintf(error_mesg, 200, "ERROR: Number of columns of inputs 8, 9, 10, and 14 are inconsistent.");
      mexErrMsgTxt(error_mesg);
      return;
    }
  ...
  for (ii = 0; ii < num_rays; ii++) {
     elevs[ii] = *(mxGetPr(prhs[0]) + ii);
  }    
  bearing = mxGetScalar(prhs[1]);
  freqs = mxMalloc(num_rays * sizeof(double));
  for (ii = 0; ii < num_rays; ii++) {
    freqs[ii] = *(mxGetPr(prhs[2]) + ii);
  }
  nhops = mxGetScalar(prhs[3]);
  radius_earth = mxGetScalar(prhs[5]);
  irregs_flag = mxGetScalar(prhs[6]);

  mrows = mxGetM(prhs[4]);
  ncols = mxGetN(prhs[4]);
  nelem = mrows*ncols;
  if (nelem == 1) {
    temp = mxGetScalar(prhs[4]);
    ...
    step_size_min = *(mxGetPr(prhs[4]) + 1);
    step_size_max = *(mxGetPr(prhs[4]) + 2);
  }
  ...
  if (initialize_iono) {
	...
	idx = jj*mrows + ii;
	ionosphere.eN[jj][ii] = *(mxGetPr(prhs[7]) + idx);
	ionosphere.eN_5[jj][ii] = *(mxGetPr(prhs[8]) + idx);
	ionosphere.col_freq[jj][ii] = *(mxGetPr(prhs[9]) + idx);
      }
    }
    ...
    for (jj = 0; jj < ncols; jj++) {
      ionosphere.irreg_strength[jj] = *(mxGetPr(prhs[13]) + jj*mrows);
      ionosphere.irreg_sma_dip[jj] = *(mxGetPr(prhs[13]) + jj*mrows + 1);
      ionosphere.irreg_sma_azim[jj] = *(mxGetPr(prhs[13]) + jj*mrows + 2);
      ionosphere.dop_spread_sq[jj] = *(mxGetPr(prhs[13]) + jj*mrows+3);
    } 

    ionosphere.nRange = mxGetN(prhs[8]);
    ionosphere.NumHt = mxGetM(prhs[8]);
    ionosphere.HtMin = mxGetScalar(prhs[10]);
    ionosphere.HtInc = mxGetScalar(prhs[11]);
    ionosphere.dRange = mxGetScalar(prhs[12]);
  ...
  raytrace_2d_sp_(&num_rays, elevs, &bearing, freqs, &nhops, &step_size_min, 
		  &step_size_max, &tol, &radius_earth, &ionosphere, 
		  &irregs_flag, &return_ray_path_data, 
                  &return_ray_state_vec, ray_data, ray_path_data, ray_label, 
		  nhops_attempted, ray_state_vec_in, npts_in_ray,
  ...
  if (new_format) {     /* copy data to Matlab structures */
    ...
    /* execution time for the raytrace call */
    plhs[3] = mxCreateDoubleMatrix((mwSize) 1, (mwSize) 1, mxREAL);
    *(mxGetPr(plhs[3])) = elapsed_time;
