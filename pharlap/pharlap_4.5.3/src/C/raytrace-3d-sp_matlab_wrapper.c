/* 
  ==============================================================================
                   raytrace-3d-sp_matlab_wrapper.c
  ==============================================================================
   Matlab wrapper for raytrace_3d_sp
        
   This is a MEX-file for MATLAB.
       
   Change history:
   27/03/15  M.A.Cervera  V1.0  Forked from raytrace_3d.f90 (V1.6) and converted
                                to spherical Earth version 
   28/04/15  M.A. Cervera V1.1  Added routine called from mexAtExit to clear
                                ionosphere when clear is called from Matlab
   03/12/15  M.A. Cervera V2.0  Now calls multi-threaded raytrace_3d_sp
   ...
   12/07/17  M.A. Cervera V2.5  Now doesn't return ray_path_data and ray_state_vec
                                arrays if not requested in the matlab call
   25/01/18  M.A. Cervera V2.6  Fixed a memory leak issue when freeing allocated
                                memory for the ionosphere via the 
                                clear_ionosphere() function
   14/06/18  M.A. Cervera V2.7  Now returns the collision frequency along the
                                ray path
...
void raytrace_3d_sp_(double *start_lat, double *start_long,double *start_height,
	       int *num_rays, double *elevs, double *bearings, double *freqs,
	       int *OX_mode, int *nhops, double *step_size_min,
	       double *step_size_max, double *tol, double* rad_earth,
	       ...
	       int *return_ray_state_vec, double *ray_data, 
               ...
               int *npts_in_ray, double *ray_state_vec_out, double *elapsed_time);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  char error_mesg[200],filename[200], *refdata_dir, *old_format, *old_format_3D;
  FILE *fopen(), *file_ptr;
  ...
  double  start_lat;            /* geocentric lat. of ray start point */
  double  start_long;           /* geocentric lon. of ray start point */
  double  start_height;         /* geocentric height of ray start point */
  ...
  double  rad_earth;            /* radius of Earth (m) to use by the  
  ...
  in_arrsize[9] = 1;               /* radius of Earth */    
  in_arrsize[10] = iono_grid_size; /* ionospheric elec density grid */
  in_arrsize[11] = iono_grid_size; /* elec density grid 5 mins later */
  in_arrsize[12] = iono_grid_size; /* ionospheric collision frequency grid */
  in_arrsize[13] = 9;              /* ionospheric grid parameters */
  in_arrsize[14] = 101*101*201;    /* gridded Bx */
  in_arrsize[15] = 101*101*201;    /* gridded By */
  in_arrsize[16] = 101*101*201;    /* gridded Bz */
  in_arrsize[17] = 9;              /* geomagnetic grid parameters */
  in_arrsize[18] = 12;             /* optional input - state vector at start */
  
  /* Check for proper number of input arguments. */
  if (nrhs != 10 && nrhs != 11 && nrhs != 18 && nrhs != 19) {
  ...
  if (nrhs == 10 || nrhs == 11) {
    ...
    /* OK since we have (nrhs == 10 || nrhs == 11) then an ionosphere has not
       ...
       raytrace_3d_sp call has already passed it in. If not return an error */
    if (!iono_exist_in_mem) {
      mexErrMsgTxt("ERROR: ionosphere does not exist in memory from a prior raytrace_3d_sp call.");
      return;
    } else {
      initialize_iono = 0;   /* use ionospheric and geomagnetic data grids from 
                                prior call to raytrace_3d_sp */
    }
  }

  /* Check to ensure the required inputs are numeric. The optional inputs are 
     checked later. */
    if (nrhs == 10 || nrhs == 18) {
    num_numeric = nrhs;
  } else {
  ...
  /* make sure inputs 6 to 9 have the correct size */
  for (ii = 6; ii < 10; ii++) {
    mrows = mxGetM(prhs[ii]);
    ncols = mxGetN(prhs[ii]);
    arrsize = mrows*ncols;
  ...
  if (initialize_iono) {
    ...
    /* make sure inputs 10 to 12 have the correct number of dimensions and
       are not too large */ 
    for (ii = 10; ii < 13; ii++) { 
      dims = mxGetDimensions(prhs[ii]);
    ...
    /* make sure array size of inputs 10 to 12 are consistent */ 
    dims_prev = mxGetDimensions(prhs[10]);
    for (ii = 11; ii < 13; ii++) { 
      dims = mxGetDimensions(prhs[ii]);
      if (dims[0] != dims_prev[0] || dims[1] != dims_prev[1] || 
	  dims[2] != dims_prev[2]) {
	snprintf(error_mesg, 200,
		"ERROR: Array size of inputs 11, 12 and 13 are inconsistent.");
	mexErrMsgTxt(error_mesg);
	return;
      }
      dims_prev = mxGetDimensions(prhs[ii]);
    ...
    /* make sure input 13 has the correct size */
    ii = 13;
    mrows = mxGetM(prhs[ii]);
    ncols = mxGetN(prhs[ii]);
    arrsize = mrows*ncols;
    ...
    /* make sure inputs 14 to 16 have the correct number of dimensions and
       are not too large */ 
    for (ii = 14; ii < 16; ii++) { 
      dims = mxGetDimensions(prhs[ii]);
      number_of_dimensions = mxGetNumberOfDimensions(prhs[ii]);
    ...
    /* make sure array size of inputs 14 to 16 are consistent */ 
    dims_prev = mxGetDimensions(prhs[14]);
    for (ii = 15; ii < 17; ii++) { 
      dims = mxGetDimensions(prhs[ii]);
      if (dims[0] != dims_prev[0] || dims[1] != dims_prev[1] || 
	  dims[2] != dims_prev[2]) {
    ...
    /* make sure input 17 has the correct size */
    ii = 17;
    mrows = mxGetM(prhs[ii]);
    ncols = mxGetN(prhs[ii]);
    arrsize = mrows*ncols;
  ...
  rad_earth = mxGetScalar(prhs[9]);

  /* if the ionosphere is required to be initialized (rather than using that 
     from a prior call to raytrace_3d_sp) then read it in from Matlab */
  if (initialize_iono) {
	...
	for (ii = 0; ii < dims[0]; ii++) {
	  ...
	  ptr_ionosphere->eN[kk][jj][ii] = *(mxGetPr(prhs[10]) + idx);
	  ptr_ionosphere->eN_5[kk][jj][ii] = *(mxGetPr(prhs[11]) + idx);
	  ptr_ionosphere->col_freq[kk][jj][ii] = *(mxGetPr(prhs[12]) + idx);
	}
      }
    }
    ptr_ionosphere->lat_min = *(mxGetPr(prhs[13]) + 0);
    ...
    ptr_ionosphere->lat_inc = *(mxGetPr(prhs[13]) + 1);
    ptr_ionosphere->num_lat = *(mxGetPr(prhs[13]) + 2);
    ...
    ptr_ionosphere->lon_min = *(mxGetPr(prhs[13]) + 3);
    ...
    ptr_ionosphere->lon_inc = *(mxGetPr(prhs[13]) + 4);
    ptr_ionosphere->num_lon = *(mxGetPr(prhs[13]) + 5);
    ...
    ptr_ionosphere->ht_min = *(mxGetPr(prhs[13]) + 6);
    ptr_ionosphere->ht_inc = *(mxGetPr(prhs[13]) + 7);
    ptr_ionosphere->num_ht = *(mxGetPr(prhs[13]) + 8);
	...
	for (ii = 0; ii < dims[0]; ii++) {
	  idx = kk*dims[1]*dims[0] + jj*dims[0] + ii;
	  geomag_field.Bx[kk][jj][ii] = *(mxGetPr(prhs[14]) + idx);
	  geomag_field.By[kk][jj][ii] = *(mxGetPr(prhs[15]) + idx);
	  geomag_field.Bz[kk][jj][ii] = *(mxGetPr(prhs[16]) + idx);
	}
      }
    }
    geomag_field.lat_min = *(mxGetPr(prhs[17]) + 0);
    ...
    geomag_field.lat_inc = *(mxGetPr(prhs[17]) + 1);
    geomag_field.num_lat = *(mxGetPr(prhs[17]) + 2);
    ...
    geomag_field.lon_min = *(mxGetPr(prhs[17]) + 3);
    ...
    geomag_field.lon_inc = *(mxGetPr(prhs[17]) + 4);
    geomag_field.num_lon = *(mxGetPr(prhs[17]) + 5);
    ...
    geomag_field.ht_min = *(mxGetPr(prhs[17]) + 6);
    geomag_field.ht_inc = *(mxGetPr(prhs[17]) + 7);
    geomag_field.num_ht = *(mxGetPr(prhs[17]) + 8);
  ...
  /* call the computational routine raytrace_3d_sp_ */
  step_size_min = step_size_min * 1000.0;         /* convert to meters */
  step_size_max = step_size_max * 1000.0;         /* convert to meters */

  raytrace_3d_sp_(&start_lat, &start_long, &start_height, &num_rays, elevs,
	          bearings, freqs, &OX_mode, &nhops, &step_size_min,
		  &step_size_max, &tol, &rad_earth, ptr_ionosphere,
		  &geomag_field, ray_state_vec_in, &return_ray_path_data, 
                  &return_ray_state_vec, ray_data, ray_path_data,
		  ray_label, nhops_attempted, npts_in_ray, ray_state_vec_out,
                  &elapsed_time);
