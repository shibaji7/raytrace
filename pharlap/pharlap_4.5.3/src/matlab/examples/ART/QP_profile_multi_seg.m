...
function [elec_dens, dN, QP_seg_coeffs]  =...
     QP_profile_multi_seg(foE, hmE, ymE, foF1, hmF1, ymF1, foF2, hmF2, ...
                          ymF2, heights, re) 
  ...
  QP_seg_coeffs(segment, 4) = rmF2;
  QP_seg_coeffs(segment, 5) = 1;   % F2 layer segment

  segment = 5;
  QP_seg_coeffs(segment, 1) = aj;
  QP_seg_coeffs(segment, 2) = -bj;
  QP_seg_coeffs(segment, 3) = rmF1;
  QP_seg_coeffs(segment, 4) = rc;
  QP_seg_coeffs(segment, 5) = -1;   % joining segment - bottom of F2 to F1
  ...
  QP_seg_coeffs(segment, 2) = b;
  QP_seg_coeffs(segment, 3) = rc;
  QP_seg_coeffs(segment, 4) = rmF1;
  QP_seg_coeffs(segment, 5) = 1;   % F1 layer segment

  segment = 3;
  QP_seg_coeffs(segment, 1) = aj;
  QP_seg_coeffs(segment, 2) = -bj;
  QP_seg_coeffs(segment, 3) = rmE;
  QP_seg_coeffs(segment, 4) = rc;
  QP_seg_coeffs(segment, 5) = -1;   % joining segment - bottom of F1 to E
  ...
  % from the segments
  %
  N_tot = zeros(size(heights));
  dN_tot = zeros(size(heights));

  % E smoothing segment
  r_E_join =  QP_seg_coeffs(1, 4);
  idx = find(heights+re < r_E_join);
  N_tot(idx) = N_E_smooth(idx);
  dN_tot(idx) = dN_E_smooth(idx);
  
  % E layer segment
  idx = find(heights+re >= r_E_join & heights+re < rmE); 
  N_tot(idx) = N_E_layer(idx);  
  dN_tot(idx) = dN_E_layer(idx);  
  
  % F1 - E joining segment
  r_F1_join =  QP_seg_coeffs(3, 4);
  idx = find(heights+re >= rmE & heights+re < r_F1_join);
  N_tot(idx) = N_F1_E_join(idx);
  dN_tot(idx) = dN_F1_E_join(idx);

  % F1 layer segment
  idx = find(heights+re >= r_F1_join & heights+re < rmF1);
  N_tot(idx) = N_F1_layer(idx);
  dN_tot(idx) = dN_F1_layer(idx);
  
  % F2 - F1 joining segment
  r_F2_join =  QP_seg_coeffs(5, 4);
  idx = find(heights+re >= rmF1 & heights+re < r_F2_join);
  N_tot(idx) = N_F2_F1_join(idx);
  dN_tot(idx) = dN_F2_F1_join(idx);

  % F2 layer segment
  idx = find(heights+re >= r_F2_join);
