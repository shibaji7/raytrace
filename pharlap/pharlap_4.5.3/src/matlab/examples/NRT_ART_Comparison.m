...
Bx = zeros(B_num_lat, B_num_lon, B_num_ht)+2e-5;
By = zeros(B_num_lat, B_num_lon, B_num_ht)+2e-6;
Bz = zeros(B_num_lat, B_num_lon, B_num_ht)-5e-5;
...
fprintf('3D NRT: generating %d ''no-field'' rays ...\n', num_elevs);
tic
group_range = zeros(1,num_elevs);
ground_range =zeros(1,num_elevs); ;
ray_data_N = ...
  raytrace_3d_sp(origin_lat, origin_long, origin_ht, elevs, ray_bears, ...
              freqs, OX_mode, nhops, tol, re*1000, iono_en_grid, ...
	      iono_en_grid_5, collision_freq, iono_grid_parms, Bx, By, Bz, ...
	      geomag_grid_parms);
NRT_total_time = toc;
time3 = [ray_data_N.NRT_elapsed_time];
fprintf('  NRT-only execution time = %f, Total mex execution time = %f\n\n', ...
        [ray_data_N.NRT_elapsed_time], NRT_total_time)

group_range = group_range +[ray_data_N(:).group_range];
ground_range = ground_range+ [ray_data_N(:).ground_range];
...
grid on
ray_apogee_3DNRT = ones(size(elevs))*NaN;
...
irregs_flag = 0;
fprintf('2D NRT: generating %d rays            ...\n', num_elevs);
tic
...
% Now do ART
%
ground_ART = NaN.*zeros(size(elevs));
group_ART = NaN.*zeros(size(elevs));
phase_ART = NaN.*zeros(size(elevs));
for ii = 1:num_elevs
  [gnd_ART, grp_ART, phas_ART] = ...
           ART_QP(elevs(ii), freqs(ii), re, QP_seg_coeffs);
...
idx = find(~isnan(ground_ART) & [ray_data_N(:).ray_label] == 1);  
...
idx = find(~isnan(ground_ART) & [ray_data(:).ray_label] == 1);  
...
idx = find([ray_data_N(:).ray_label] == 1 & [ray_data(:).ray_label] == 1);  
