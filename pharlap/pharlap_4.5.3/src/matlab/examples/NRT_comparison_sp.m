...
%   fan of rays in an ionosphere with a down range gradient (no cross-range 
%   gradients).
%
% Calling sequence :
%   NRT_comparison_sp
...
% setup general stuff
%
UT = [2000 1 15 6 0];              % UT - year, month, day, hour, minute
...
re = 6376.0;                       % radius of the Earth to use

close all

fprintf(['\n' ...
   'Comparison of 3D NRT ("no-field") and 2D NRT for a spherical Earth\n'])
fprintf( ...
  'The ionosphere used has down-range gradients but NO cross-range \n')
fprintf('gradients.\n')
fprintf('Earth radius used is %f km\n\n', re)
...
%   [lat, lon] = raz2latlon(range, azim, origin_lat, origin_long, ...
%                           'spherical', re*1000);
%   
...
tic
ray_data = ...
    raytrace_2d_sp(elevs, ray_bears(1), freqs, nhops, ...
                tol, re, irregs_flag, iono_en_grid_2D, iono_en_grid_5_2D, ...
