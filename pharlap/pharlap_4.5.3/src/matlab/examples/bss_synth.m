...
%   gain patterns and receiver mis-match loss are not included (search for
%   gain_tx, gain_rx and rx_mis_match_loss to see where they should be
%   included). 
%
%   Absorption returned by raytrace_2d (SiMIAN) or calculated using the George
%   and Bradley absorption model for O-mode propagation may be used by setting
%   GB_abso_flag accordingly (= 0 for SiMIAN). Note SiMIAN returns a slightly
%   larger value of absorption as it has be calculated from the 2D NRT which
%   "no magnetic field" and does not consider O or X modes.  
%   X absorption > SiMIAN 2D absorption > O absorption (see abso_comp_2dnrt.m)
%
% Calling sequence :
%   bss_synth
%
...
%
%   V1.6  M.A. Cervera  24/10/2018
%      Updated to enable SiMIAN to be used for the absorption model
%
%   V1.7  M.A. Cervera  01/06/2020
%      Corrected an error with the (non-default) George-Bradley absorption
%      calculation for second and greater hops (first hop propagation was not
%      affected). Improved calculation of spread of flux tubes.
%
%   V1.8  M.A. Cervera  16/05/2023
...
GB_abso_flag = 0;             % use George and Bradley absorption
...
num_modes = zeros(num_groups, num_freqs);
...
if GB_abso_flag
  ...
  fprintf('Generating the model BSS using SiMIAN absorption... ')
end
...
for freq = freq_min:freq_step:freq_max
  ...
  dev_absorption = zeros(num_elevs, num_hops) * nan;    % deviative absorption 
  ...
  for elev_idx = 1:num_elevs 
    ...
    absorption(elev_idx, 1:nhops_done) = ...
    ...
    % George and Bradley absorption loss for the last hop
    hop_idx = hop_idx + 1;
  ...
  absorp_gb_dev = absorption_gb_dev(idx_raygnd);   % G-B + dev absorption model
  absorp_simian = absorption(idx_raygnd);          % SiMIAN absorption model
  ...
  for ray_out = 1:length(gnd_rng)
    ...
    for ray_in=1:length(gnd_rng)-1  
      ...
      ground_range_diff = abs(gnd_rng(ray_in+1) - gnd_rng(ray_in));
	...
	ray_power_dB = pow_dB - fs_loss_out - fs_loss_in - ...
	    rx_mis_match_loss - absorp_out - absorp_in;
	...
	group_end = min([group_end group_max]);
 	...
 	group_start = min(grp_rng(idx));
 	group_end = max(grp_rng(idx));
 	%if (group_end - group_start) > 600
 	%  group_end = group_start + group_step;
 	%end
