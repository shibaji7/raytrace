...
% setup general stuff
%
UT = [2000 1 15 7 0];         % UT - year, month, day, hour, minute
...
% set the elevation step size - a smaller value will "fill in" the high ray 
% better at the expense of computation time - try a value of 0.1
elev_step = 0.25;               
...
fprintf('Generating the model OIS... ')
...
while (freq <= end_freq) 
  ...
  if length(idx_goodray) > 3
	...
	if isempty(find(fine_label < 1))
	  ...
	  % Use George and Bradley absorption rather than the "built-in" 
	  % absorption calculation of raytrace_2d. This is because the 2D NRT 
	  % ignores the geomagnetic field and so the calculated absorption is 
	  % greater than that for O-mode propagation and less than that for 
	  % X-mode propagation. Note the deviative absorption term will need
	  % to be added as George and Bradley does not account for it above the
          % E Layer.
...
% Ionospheric absorption terms for O and X modes. Need to add the deviative 
% absorption term as George and Bradley does not account for it above the
% E Layer.
...
set(get(cb, 'ylabel'), 'String', 'Power (dBW)', 'fontsize', 14)
grid on
...
c_ticklabel = num2str([min_barrange:c_tick_step:max_barrange]');
cb = colorbar('ytick', c_tickval, 'yticklabel', c_ticklabel);
set(get(cb, 'ylabel'), 'String', 'Elevation (degrees)', 'fontsize', 14)
grid on
...
c_ticklabel = num2str([min_barrange:c_tick_step:max_barrange]');
cb = colorbar('ytick', c_tickval, 'yticklabel', c_ticklabel);
set(get(cb, 'ylabel'), 'String', 'Ray Apogee (km)', 'fontsize', 14)
grid on