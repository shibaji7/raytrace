...
%
%   V1.5  M.A. Cervera  20/05/2017
%      Fixed bug where the input hour and minute was not used correctly
...
while(strcmp(synth_iono, 'Yes'))
  ...
  UT(3) = 15;     % day 15 - middle of the month
  UT(4) = str2num(hour_str);
  UT(5) = str2num(minute_str);
  switch UT(2)
  ...
  while(pass_num < 2)
    ...
    while (freq <= end_freq)
      ...
      ray_data = raytrace_2d(tx_lat, tx_lon, elevs, ray_bearing, freqs,  ...
      ...
      for hop_idx = 1:nhops
	    ...
	    % far into a cusp region to be reliable
            if ((gnd(ii) >= range_rx & gnd(ii+1) < range_rx) | ...
              ...
              % quantities 
	      frac = (range_rx - gnd(ii)) ./ (gnd(ii+1) - gnd(ii));
	      elev_torx = els(ii) + (els(ii+1) - els(ii)) .* frac; 

	      ray_data2 = raytrace_2d(tx_lat, tx_lon, elev_torx, ...
		       ray_bearing, freq, hop_idx, tol, irregs_flag);

	      if ray_data2.ray_label == 1
		elevation = [elevation elev_torx];
		group_range = [group_range ray_data2.group_range(hop_idx)];
		phase_path =  [phase_path ray_data2.phase_path(hop_idx)];
		dev_absorption = ...
		    [dev_absorption ray_data2.deviative_absorption(hop_idx)];
		eff_range = ...
		    [eff_range ray_data2.effective_range(hop_idx)];

		gnd_fs_loss = 0;
		O_absorp = 0;
		X_absorp = 0;
		for kk = 1:hop_idx
		  ray_apogee  = ray_data2.apogee(kk);
		  ray_apogee_gndr  = ray_data2.gnd_rng_to_apogee(kk);
		  [ray_apogee_lat, ray_apogee_lon] = raz2latlon( ...
		      ray_apogee_gndr, ray_bearing, tx_lat, tx_lon, 'wgs84');
		  plasfrq_at_apogee = ray_data2.plasma_freq_at_apogee(kk);
		  if kk == 1
		  ...
		  % Use George and Bradley absorption rather than the  
		  % "built-in" absorption calculation of raytrace_2d. This
		  % is because the 2D NRT ignores the geomagnetic field and
		  % so the calculated absorption is greater than that for
		  % O-mode propagation and less than that for X-mode
		  % propagation. Note the deviative absorption term will
		  % need to be added as George and Bradley does not account
		  % for it above the E Layer.
		  ...
		  if kk > 1
		    fs_lat = ray_data2.lat(kk-1);
		    fs_lon = ray_data2.lon(kk-1);
  ...
  if no_propagation
    ...
    % ionospheric absorption terms for O and X modes. Need to add the
    % deviative absorption term as George and Bradley does not account for it
    % above the E Layer.
    ...
    title([month_str ' ' UT_str '       SSN ' SSN_str ...
      '     1 Watt radiator, isotropic antennas       tx lat ' ...
