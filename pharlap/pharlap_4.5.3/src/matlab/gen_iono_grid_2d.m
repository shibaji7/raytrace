...
%
%   V1.7  M.A. Cervera 06/06/2019
%     Removed the "Chapman" options. No longer required. 
%
%   V1.8  M.A. Cervera 24/02/2020
...
%
%   V1.9  M.A. Cervera 16/05/2023
%     Now uses IRI2020 and IGRF2020
%
...
function [iono_pf_grid,iono_pf_grid_5,collision_freq,irreg,iono_te_grid] = ...
  ...
  parfor rng = 1:num_range
    ...
    % neutral densities
    lat_arr = lat * ones(size(height_arr));
    lon_arr = lon * ones(size(height_arr));
    if (R12 == -1)
    ...
    mag_field = igrf2020(lat, lon, UT, 300);
    dip = mag_field(8);
    dec = mag_field(10);
...
function [iono_pf_prof,iono_pf_prof5,iono_extra,iono_te_prof,iono_ti_prof] = ...
      gen_iono_profile(lat, lon, num_heights, start_height, height_inc, ...
		       ...
		       profile_type, iri_options, doppler_flag);
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % This is IRI2020 with or without FIRI option %
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  if (strcmp(lower(profile_type), 'iri') | ...
	  strcmp(lower(profile_type), 'iri2020') | ...
	  strcmp(lower(profile_type), 'firi2020') )
    
    firi_flag = strcmp(lower(profile_type), 'firi2020') | ...
	        strcmp(lower(profile_type), 'firi');

    if (firi_flag)
      % call IRI 2020, with FIRI option on
      [iono, iono_extra] = iri2020_firi_interp(lat, lon, R12, UT, ...
	           start_height, height_inc, num_heights, iri_options);

      if doppler_flag
	[iono5, iono_extra5] = iri2020_firi_interp(lat, lon, R12, ...
      ...
      if doppler_flag
	  [iono5, iono_extra5] = iri2020(lat, lon, R12, UT_5, ...
	             start_height, height_inc, num_heights, iri_options);
      end
    end
  ...
  elseif (strcmp(lower(profile_type), 'iri2016') | ...
    ...
    firi_flag = strcmp(lower(profile_type), 'firi2016');
