...
%
% Purpose :
%   Matlab wrapper to the IRI-2020 fortan based empirical model ionosphere. 
...
%      R12 = 1 - 200 :  IRI2020 is called with R12 (Zurich V1.0) input as the
...
%      R12 = -1      :  IRI2020 is called with ionospheric conditions (R12,
...
%                     IRI2020 determines the B0 (thickness) and B1 (shape)
...
%
%       .B0   - user input bottomside thickness (km). Leave undefined to
%               use one of the built-in IRI models specified by the field
%               .Ne_B0B1_model (see above). Specifing this field will
%               override the field .Ne_B0B1_model .
%
%       .B1   - user input bottomside profile shape. Leave undefined to use one
%               of the built-in IRI models specified by field .Ne_B0B1_model 
%               (see above). Valid values : 0.6 < B1 < 6, values outside this 
%               range are clipped. B1 will be ignored if B0 has not also been 
%               specified.
%
...
%                    'FPT-2018' - Friedrich, Pock and Torkar's FIRI-2018 model 
%                                 for the lower ionosphere (JGR: Space, 123,
%                                 6737, doi.org/10.1029/2018JA025437).
%                                 Danilov et al. D-region model
...
%
%       .CGM_comp - specifies whether or not to turn CGM computation on
%                      'off' - CGM computation off  (default)
%                      'on'  - CGM computation on
%
%       .Ti_model - specifies the ion temperature model. Valid values are:
%                      'Tru-2021'  - Truhlik et al. 2021 empirical model (default)
%                      'Bil-1981'  - Bilitza 1981 model
%
%       .plasmasphere - specifies the plasmasphere model:
%                          'Ozhogin'   - Plasmasphere model of Ozhogin etal. (2012) up to plasmapause
%                          'Gallagher' - Plasmasphere model of Gallager et al. (2000) up to plasmapause
%
% Outputs :
%   iono  -  Array of 15 output quantities x num_hts heights. NB, ionosperic
...
%                    If iri_options.D_model = 'FPT-2018' then Ne is given by
%                    the Friedrich, Pock and Torkar (2018) FIRI model in the
%                    height interval 60 - 140 km
...
%               78:  ) = NOT USED (value = -1)
%       iono(15, :) = plasma frequency divided by gyro frquency
%
...
%                by a value of -1. Consult irisub.for (obtained from website
%                irimodel.org) for the explanation of these parameters.
...
%      iono_extra(27) = modified dip lat.  iono_extra(28) = Geographic latitude
...
%      iono_extra(53) = invdip/degree      iono_extra(54) = MLT-Te
%      iono_extra(55) = CGM-latitude       iono_extra(56) = CGM-longitude
%      iono_extra(57) = CGM-MLT            iono_extra(58) = CGM lat eq. aurl bodry
%      iono_extra(59) = CGM-lati(MLT=0)    iono_extra(60) = CGM-lati for MLT=1
%      iono_extra(61) = CGM-lati(MLT=2)    iono_extra(62) = CGM-lati for MLT=3
%      iono_extra(63) = CGM-lati(MLT=4)    iono_extra(64) = CGM-lati for MLT=5
%      iono_extra(65) = CGM-lati(MLT=6)    iono_extra(66) = CGM-lati for MLT=7
%      iono_extra(67) = CGM-lati(MLT=8)    iono_extra(68) = CGM-lati for MLT=9
%      iono_extra(69) = CGM-lati(MLT=10)   iono_extra(70) = CGM-lati for MLT=11
%      iono_extra(71) = CGM-lati(MLT=12)   iono_extra(72) = CGM-lati for MLT=13
%      iono_extra(73) = CGM-lati(MLT=14)   iono_extra(74) = CGM-lati for MLT=15
%      iono_extra(75) = CGM-lati(MLT=16)   iono_extra(76) = CGM-lati for MLT=17
%      iono_extra(77) = CGM-lati(MLT=18)   iono_extra(78) = CGM-lati for MLT=19
%      iono_extra(79) = CGM-lati(MLT=20)   iono_extra(80) = CGM-lati for MLT=21
%      iono_extra(81) = CGM-lati(MLT=22)   iono_extra(82) = CGM-lati for MLT=23
%      iono_extra(83) = Kp at current time iono_extra(84) = magnetic declination
%      iono_extra(85) = L-value            iono_extra(86) = dipole moment 
%      iono_extra(87) = SAX300             iono_extra(88) = SUX300 
%      iono_extra(89) = HNEA               iono_extra(90) = HNEE 
%      iono_extra(91 - 100) = Unused
%
%  Notes :
%   1. Notes for IRI2020 called using specified input ionospheric conditions:
...
%          [iono iono_extra] = iri2020(-25, 135, 70, [2000 1 1 3 0])
%       returns NmF2 = 1.0252e+12 electrons/m^3, whereas
%          [iono iono_extra] = iri2020(-25, 135, 70, [2001 1 1 3 0])
%       returns NmF2 = 1.0260e+12
%
%   1.2 User defined IG12, F10.7 and F10.7_81 (3 solar rotation average of
%       F10.7) required by IRI2020 are derived from R12 using the following
%       empirical formulas : 
...
%       IRI-2020
%
%   2. Notes for IRI2020 called using specified input epoch:
%   2.1 IRI2020 uses solar indices tabled in ig_rz.dat (which is supplied with
%       IRI2020). This file contains R12 and IG12 from Jan 1958 to Dec 2018. If 
...
%
%   2.3 IRI indices files ig_rz.dat and apf107.dat updated 05-May-2023
%
%   2.4 The solar activity parameter F10.7 (daily) and magnetic activity Ap
%       index (3-hourly) used by IRI2020 are tabled in apf107.dat from 1 Jan 
...
%       F10.7. 
%
%   3. This mex file drives IRI-2020 with the following default options
...
%      3.45 <unused>
%      3.46 <unused>
%      3.47 CGM computation off           
%      3.48 Ti  Tru-2021    
%      3.49 Plasmasphere: Ozhogin model
%      3.50 <unused>
...
%   V1.0  M. A. Cervera  16/05/2023
%      Initial version
%   
%

% This a Matlab help file only. The actual programme is a mex wrapper
% (iri2020_matlab_wrapper.c) to the Fortran code (irisub.for).
