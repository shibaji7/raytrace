%
% Name :
%   iri2020_firi_interp.m
%
% Purpose :
%   Matlab wrapper to the IRI2020 Fortan based empirical model ionosphere,
%   using Friedrich, Pock and Torkar's FIRI-2018 model for the lower 
%   ionosphere (JGR: Space, 123, 6737, doi.org/10.1029/2018JA025437). FIRI 
%   replaces the standard IRI below 140 km. However, a discontinuity occurs
%   at 14Okm when the FIRI-2018 option is selected when calling IRI2020.
%   This wrapper interpolates between 120km of the FIRI model and the 
%   F layer of the standard IRI model.
%
...
%
% Calling sequence :
%    1. [iono, iono_extra] = iri2020_firi_interp(lat, lon, R12, UT, ...
%                                 ht_start, ht_step, num_hts);
%
%    2. [iono, iono_extra] = iri2020_firi_interp(lat, lon, R12, UT, ...
%                                 ht_start, ht_step ,num_hts, iri_options);
%
% Inputs :
...
%                     'none' - no F1 layer
%
%       .D_model - specifies the model to use for the D Layer. However, the
%                  input value is overridden and FT-2001 is always used:
%                  'FT-2001'  - Friedrich and Torkar's FIRI model for the 
%                               lower ionosphere (Friedrich and Torkar,
%                               J. Geophys Res. 106 (A10), 21409Ð21418,
...
%
%   2.3 ig_rz.dat updated 20-Feb-2016
%
...
%      3.24 D-Region model - FT-2001 (can nor be changed by user input)
...
%      Based on V1.3  of iri2016_firi_interp.m
%
...
function [iono, iono_extra] = iri2020_firi_interp(site_lat, site_lon, R12, ...
  ...
  % override the input iri_options to make sure FIRI D-layer is selected
  iri_options.D_model = 'FPT-2018';       
  
  iri2020_arg_str = ['site_lat, site_lon, R12, UT, start_height, ' ...
		  'height_step, num_heights, iri_options'];
 
  iricall_str = ['[iono iono_extra] = iri2020(' iri2020_arg_str ');'];
