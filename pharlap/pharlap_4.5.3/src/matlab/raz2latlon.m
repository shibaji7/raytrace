...
%   particular origin on the Earth to latitude and longitude.
%	
% Calling sequence:
%   1.  [geo_lat, geo_long] = raz2latlon(range, azim, origin_lat, origin_long)
%   2.  [geo_lat, geo_long] = raz2latlon(range, azim, origin_lat, ...
%                                        origin_long, geoid)
%   3.  [geo_lat, geo_long] = raz2latlon(range, azim, origin_lat, ...
%                                        origin_long, 'wgs84')
%   4.  [geo_lat, geo_long] = raz2latlon(range, azim, origin_lat, ...
%                                        origin_long, 'spherical')
%   5.  [geo_lat, geo_long] = raz2latlon(range, azim, origin_lat, ...
%                                        origin_long, 'spherical', re)
%
%
% Inputs:	
%   range 	Array	range (m) of point from the origin.
%   azim 	Array	azimuth of point from true North with respect to the 
%                       origin (degrees).
%   origin_long	Scalar	longitude of origin (degrees).	
%   origin_lat	Scalar	latitude of origin (degrees).
%
% Optional Inputs
...
switch lower(vargin)
   case {'spherical', ''}       % spherical Earth
     ...
     geo_long   = radeg*atan2(y,x) + origin_long;
  
  
  otherwise    % ellipsoid
    %---------  Initial values  --------------------
    epsi = 0.5e-13;			% Tolerence.
    glon1 = origin_long * dtor;
